...
│   └── versions/
│       └── 4ee79f900dc6_create_alert_registrations_table.py
├── alembic.ini     ← points at sqlite:///./riobus.db
└── riobus.db       ← the actual SQLite database
```
...
import models  # registers tables on Base.metadata — the import itself is the side effect
target_metadata = Base.metadata
```

`Base` appears empty in `database.py`, but importing `models` executes `class AlertRegistration(Base)`,
which registers the table on `Base.metadata` automatically. Every new model file added in the future
must also be imported here, or Alembic will not detect it.

### Important: never use `Base.metadata.create_all()` alongside Alembic

Early in the project `main.py` contained `Base.metadata.create_all(bind=engine)` as a convenience.
This caused the first autogenerated migration to be empty — Alembic compared the models against
the DB, saw the table already existed (created by `create_all`), and generated a `pass` migration.
The line was removed and the migration was regenerated from a clean DB. **Alembic is the sole owner
of schema creation** — `create_all` bypasses version control and causes silent empty diffs.
